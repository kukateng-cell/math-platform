...
## 一句話定位
K-2 數學學習平台（Next.js 16 App Router + Prisma 7 + SQLite）。家長建檔、孩子做題、遊戲化（星星/連續/徽章）、規則式掌握度推薦、Admin 後台。
...
- **SQLite**（`better-sqlite3`），adapter 為 `@prisma/adapter-better-sqlite3`
