...
function hashOtp(identifier: string, code: string): string {
  return createHmac('sha256', OTP_TEST_KEY).update(`${identifier}:${code}`).digest('hex')
}

describe('OTP 雜湊邏輯（防 timing attack）', () => {
  it('相同 identifier + code 產生相同 hash', () => {
    const h1 = hashOtp('user1', '123456')
    const h2 = hashOtp('user1', '123456')
    expect(h1).toBe(h2)
  })

  it('不同 identifier 產生不同 hash', () => {
    const h1 = hashOtp('user1', '123456')
    const h2 = hashOtp('user2', '123456')
    expect(h1).not.toBe(h2)
  })

  it('不同 code 產生不同 hash', () => {
    const h1 = hashOtp('user1', '123456')
    const h2 = hashOtp('user1', '654321')
    expect(h1).not.toBe(h2)
  })

  it('hash 長度固定為 64（SHA256 hex）', () => {
    const h = hashOtp('user1', '123456')
    expect(h).toHaveLength(64)
  })

  it('不會包含原始 code 或 identifier', () => {
    const h = hashOtp('user@example.com', '123456')
