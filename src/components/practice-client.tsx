...
export default function PracticeClient({
  ...
  if (index >= questions.length || (lastResult?.finished && index === questions.length - 1)) {
    ...
    return (
      <div className="flex flex-col items-center gap-5 px-2 text-center sm:gap-6" role="region" aria-label="練習完成">
        <div className="text-5xl sm:text-6xl">{encouragement.emoji}</div>
        <h2 className="text-xl font-bold sm:text-2xl">{childNickname} 完成了！</h2>
        <p className="text-base font-medium text-indigo-600 dark:text-indigo-400 sm:text-lg">{encouragement.emoji} {encouragement.msg}</p>

        {/* 答對題數 + 總花費時間 */}
        <div className="flex flex-wrap items-center justify-center gap-x-4 gap-y-1 sm:gap-x-6">
          <p className="text-sm text-neutral-600 dark:text-gray-300 sm:text-lg">
        ...
        <div className="w-full max-w-xs sm:max-w-md">
          <div className="mb-1 flex justify-between text-xs text-neutral-500 dark:text-gray-400 sm:text-sm">
            <span>正確率</span>
            <span className="font-bold text-green-600 dark:text-green-400">{accuracy}%</span>
          </div>
          <div className="h-2.5 w-full rounded-full bg-neutral-200 dark:bg-gray-700 sm:h-3" role="progressbar" aria-valuenow={accuracy} aria-valuemin={0} aria-valuemax={100} aria-label={"正確率 " + accuracy + "%"}>
            <div
              className="h-2.5 rounded-full bg-gradient-to-r from-green-400 to-green-600 transition-all duration-700 sm:h-3"
        ...
        {starsEarned > 0 && (
          <div className="my-1 sm:my-2">
            ...
            <p className="mt-2 text-xs font-medium text-amber-600 dark:text-amber-400 sm:text-sm">
              +{starsEarned} 顆星星 ⭐
            </p>
          </div>
        )}
        ...
        {/* 每題結果一覽 */}
        <div className="w-full max-w-xs sm:max-w-md">
          <h3 className="mb-2 text-xs font-semibold text-neutral-500 dark:text-gray-400 sm:text-sm">每題結果一覽</h3>
          <ol className="flex flex-col gap-1">
            {questions.map((q, i) => {
              ...
              return (
                <li key={i} className={"flex items-center justify-between rounded-lg border px-2.5 py-1.5 text-xs sm:px-3 sm:py-2 sm:text-sm " + cls}>
                  <span className="flex items-center gap-1.5 sm:gap-2">
                    <span className="text-sm sm:text-base">{icon}</span>
                    <span className="text-neutral-500 dark:text-gray-400">第 {i + 1} 題</span>
                  </span>
                  <span className="flex items-center gap-1.5 sm:gap-2">
                    {detail && <span className="hidden text-xs opacity-70 sm:inline">{detail}</span>}
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:gap-3">
          <a
            ...
            className="rounded-lg bg-blue-600 px-5 py-2.5 text-sm font-medium text-white transition hover:bg-blue-700 sm:text-base"
          >
            查看學習概覽
          </a>
          <a
            href={"/practice/" + childId}
            className="rounded-lg border border-neutral-300 px-5 py-2.5 text-sm font-medium transition hover:bg-neutral-50 dark:border-gray-600 dark:text-white dark:hover:bg-gray-800 sm:text-base"
          >
            再練一次
          </a>
        </div>
  ...
  return (
    <div className={"mx-auto flex w-full max-w-lg flex-col gap-4 px-1 sm:gap-6 " + bgFlashClass} onKeyDown={handleKeyDown}>
      <div>
        ...
        <div
          className="h-1.5 w-full rounded-full bg-neutral-200 dark:bg-gray-700 sm:h-2"
          role="progressbar"
        ...
        >
          <div
            className="h-1.5 rounded-full bg-gradient-to-r from-blue-500 to-indigo-500 transition-all duration-500 sm:h-2"
            style={{ width: progress + "%" }}
          />
        </div>

        {/* 進度圓點狀態指示器 */}
        <div className="mt-2 flex justify-center gap-1.5 sm:mt-3" aria-hidden="true">
          {questions.map((_, i) => {
            let dotCls = 'h-2.5 w-2.5 rounded-full border-2 border-neutral-300 bg-transparent dark:border-gray-600 sm:h-3 sm:w-3'
            let icon: string | null = null
            if (i < questionResults.length) {
              const r = questionResults[i]
              if (r.assisted) {
                dotCls = 'flex h-2.5 w-2.5 items-center justify-center rounded-full border-2 border-amber-400 bg-amber-400 text-[6px] text-white sm:h-3 sm:w-3 sm:text-[8px]'
              } else if (r.correct) {
                dotCls = 'flex h-2.5 w-2.5 items-center justify-center rounded-full border-2 border-green-500 bg-green-500 text-[6px] text-white sm:h-3 sm:w-3 sm:text-[8px]'
                icon = '✓'
              } else {
                dotCls = 'flex h-2.5 w-2.5 items-center justify-center rounded-full border-2 border-red-400 bg-red-400 text-[6px] text-white sm:h-3 sm:w-3 sm:text-[8px]'
                icon = '✗'
              }
            } else if (i === index) {
              dotCls = 'h-2.5 w-2.5 rounded-full border-2 border-blue-500 bg-blue-500 animate-pulse-dot sm:h-3 sm:w-3'
            }
            return (
      ...
      <div
        className="rounded-2xl border border-neutral-200 bg-white px-4 py-6 text-center shadow-sm dark:border-gray-700 dark:bg-gray-900 sm:px-8 sm:py-8"
        role="region"
        aria-label={"題目 " + (index + 1)}
      >
        <p className="text-2xl font-bold tracking-wide sm:text-3xl">{current.prompt}</p>
      </div>
          ...
          {current.options.map((opt, optIdx) => {
            ...
            return (
              <button
                ...
                className={"rounded-xl border-2 px-3 py-4 text-xl font-bold transition sm:px-4 sm:py-5 sm:text-2xl " + cls}
              >
                <span className="inline-flex items-center gap-1.5 sm:gap-2">
                  {opt}
                  {!lastResult && (
                    <span className="text-[10px] text-neutral-400 dark:text-gray-500 sm:text-xs">({optIdx + 1})</span>
                  )}
      ...
      {error && (
        <div className="rounded-xl bg-red-50 p-3 text-center text-xs text-red-600 dark:bg-red-950 dark:text-red-400 sm:text-sm" role="alert">
          {error}
        </div>
      )}
      ...
      {lastResult && (
        <div
          className={"animate-fade-in-up rounded-xl p-3 text-center sm:p-4 " + (lastResult.correct ? 'bg-green-50 text-green-700 dark:bg-green-950 dark:text-green-300' : 'bg-red-50 text-red-700 dark:bg-red-950 dark:text-red-300')}
          role="alert"
        >
          ...
          {lastResult.explanation && (
            <p className="mt-1 text-xs opacity-80 sm:mt-2 sm:text-sm">💡 {lastResult.explanation}</p>
          )}
        </div>
      )}

      {interaction !== 'fillin' && (
        <label className="flex items-center justify-center gap-2 text-xs text-neutral-500 dark:text-gray-400 sm:text-sm">
          <input
            type="checkbox"
            checked={assisted}
        ...
        {!lastResult ? (
          interaction !== 'fillin' ? (
            <button
              ...
              className="w-full rounded-lg bg-blue-600 px-6 py-3 text-sm font-medium text-white transition hover:bg-blue-700 disabled:opacity-40 sm:w-auto sm:px-8 sm:text-base"
            >
              {submitting ? '送出中…' : '送出答案'}
            </button>
          ) : null
          ...
          <button
            ...
            className="w-full rounded-lg bg-blue-600 px-6 py-3 text-sm font-medium text-white transition hover:bg-blue-700 sm:w-auto sm:px-8 sm:text-base"
          >
            下一題 →
          </button>
        )}
