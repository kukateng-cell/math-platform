  ...
  if (!open) {
    return (
      <div className="flex items-center gap-2 text-xs">
        {currentPin ? (
          <>
            <span className="rounded bg-green-50 px-1.5 py-0.5 text-green-600 font-mono">
              PIN: {currentPin}
            </span>
          ...
          <button
            ...
            className="rounded border border-dashed border-neutral-300 px-2 py-0.5 text-xs text-neutral-400 hover:border-blue-400 hover:text-blue-500"
          >
            + 設定 PIN
          </button>
        )}
  ...
  return (
    <div className="rounded-lg border border-neutral-200 bg-neutral-50 p-3 text-xs">
      ...
      <div className="mb-2 flex gap-1.5">
        {pin.map((d, i) => (
          <input
            ...
            className="h-9 w-8 rounded-lg border border-neutral-300 text-center text-lg font-bold outline-none focus:border-blue-500"
            autoFocus={i === 0}
          />
        ))}
      </div>
      ...
      <div className="flex gap-2">
        ...
        <button onClick={() => { setOpen(false); setError(null) }} className="rounded border border-neutral-300 px-3 py-1 hover:bg-neutral-100">
          取消
        </button>
      </div>
    </div>
