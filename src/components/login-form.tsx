...
export default function LoginForm({ initialCaptcha }: Props) {
  ...
  if (!isOtpMode) {
    return (
      <form action={loginAction} className="flex w-full max-w-sm flex-col gap-3 sm:gap-4">
        <div className="flex flex-col gap-1">
          <label htmlFor="email" className="text-sm font-medium">Email</label>
          <input id="email" name="email" type="email" placeholder="parent@example.com"
            className="rounded-lg border border-neutral-300 px-3 py-2 text-sm outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 sm:text-base" required />
          {loginState?.errors?.email && <p className="text-xs text-red-500 sm:text-sm">{loginState.errors.email[0]}</p>}
        </div>
        ...
        <div className="flex flex-col gap-1">
          ...
          <input id="password" name="password" type="password"
            className="rounded-lg border border-neutral-300 px-3 py-2 text-sm outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 sm:text-base" required />
          {loginState?.errors?.password && <p className="text-xs text-red-500 sm:text-sm">{loginState.errors.password[0]}</p>}
        </div>
        ...
        <div className="rounded-lg border border-neutral-200 bg-neutral-50 p-3 dark:border-gray-600 dark:bg-gray-800">
          ...
          <p className="mb-2 text-center text-base font-bold sm:text-lg">{captcha.question}</p>
        ...
        {loginState?.message && (
          <p className="rounded-lg bg-red-50 px-3 py-2 text-xs text-red-600 sm:text-sm">{loginState.message}</p>
        )}
  ...
  return (
    <div className="flex w-full max-w-sm flex-col gap-3 sm:gap-4">
      <form action={otpAction} className="flex flex-col gap-3 sm:gap-4">
        ...
        <div className="rounded-lg border border-blue-200 bg-blue-50 p-3 text-center dark:border-blue-800 dark:bg-blue-950 sm:p-4">
          ...
          <p className="text-xs text-blue-800 dark:text-blue-200 sm:text-sm">{loginState?.message || resendState?.message}</p>

          {/* ═══ 開發模式：直接顯示驗證碼 ═══ */}
          {displayOtp && (
            <div className="mt-2 rounded-xl bg-white/80 px-3 py-2 sm:mt-3 sm:px-4 sm:py-3">
              <p className="mb-1 text-[10px] text-blue-500 sm:text-xs">🔧 開發模式 — 驗證碼</p>
              <p className="mb-1 select-all text-2xl font-bold tracking-[0.3em] text-blue-900 dark:text-blue-100 sm:mb-2 sm:text-3xl">
                {displayOtp}
              </p>
              <button
                type="button"
                ...
                className="rounded-lg bg-blue-600 px-2.5 py-1 text-[10px] font-medium text-white hover:bg-blue-700 sm:px-3 sm:text-xs"
              >
                一鍵填入
              </button>
            </div>
          ...
          {!displayOtp && (
            <p className="mt-1 text-[10px] text-blue-600 sm:text-xs">請查看您信箱中的驗證碼</p>
          )}
        </div>

        <div className="flex flex-col gap-1">
          <label htmlFor="otpCode" className="text-xs font-medium text-center sm:text-sm">輸入 6 位數驗證碼</label>
          <input id="otpCode" name="otpCode" type="text" inputMode="numeric"
            autoComplete="one-time-code" maxLength={6} placeholder="000000"
            className="rounded-lg border border-neutral-300 px-3 py-2.5 text-center text-xl tracking-[0.5em] outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-900 dark:text-white sm:py-3 sm:text-2xl" required />
        </div>

        {otpState?.message && (
          <p className="rounded-lg bg-red-50 px-3 py-2 text-xs text-red-600 sm:text-sm">{otpState.message}</p>
        )}
      ...
      <form action={resendAction} className="text-center">
        <input type="hidden" name="tempToken" value={tempToken} />
        {countdown > 0 ? (
          <p className="text-[10px] text-neutral-400 dark:text-gray-500 sm:text-xs">
            未收到驗證碼？{countdown} 秒後可重新發送
          </p>
        ) : (
          <button type="submit" disabled={resendPending}
            className="text-xs font-medium text-blue-600 transition hover:text-blue-800 disabled:opacity-50 dark:text-blue-400 dark:hover:text-blue-300 sm:text-sm">
