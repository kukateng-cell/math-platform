    ...
    <div className="w-full px-2 py-6">
      ...
      >
        ...
        <div className="relative mt-1" style={{ height: '24px' }}>
          {marks.map((m) => {
            ...
            return (
              ...
              >
                <div className="mb-1 h-2 w-0.5 bg-neutral-300" />
                <span className="text-xs text-neutral-400 select-none">{m}</span>
              </div>
            )
          })}
        </div>
        ...
        {value !== null && (
          ...
          >
            <div className="h-7 w-7 rounded-full border-4 border-blue-500 bg-white shadow-md" />
      ...
      {/* 最小值/最大值標籤 */}
      <div className="flex justify-between px-0.5 text-xs text-neutral-400">
        <span>{min}</span>
        <span>{max}</span>
      </div>
    </div>
